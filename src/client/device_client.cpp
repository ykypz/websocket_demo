...
DeviceClient::DeviceClient() : m_done(false) {
    // 初始化WebSocket客户端
    m_client.init_asio();
...
// 开始视频流
CommandResult DeviceClient::startStream() {
    return sendCommand(CommandType::StartStream);
}

// 停止视频流
CommandResult DeviceClient::stopStream() {
    return sendCommand(CommandType::StopStream);
}

// 开始测量
CommandResult DeviceClient::executeMeasurement() {
    return sendCommand(CommandType::ExcuteMeasurement);
}

// 停止测量
CommandResult DeviceClient::stopMeasure() {
    return sendCommand(CommandType::StopMeasure);
}
...
// 获取面形数据
CommandResult DeviceClient::getSurfaceData() {
    return sendCommand(CommandType::GetSurfaceData);
}
